...
async function carregarStatus() {
    try {
        ...
        if (err.name === "TypeError" && /fetch/i.test(err.message || "")) {
            dica = " (provável bloqueio de CORS ou rede — não é erro do backend em si)";
        }

        // Mostra o erro exato na tela, sem precisar de console do navegador
        setStatus(`⚠️ Falha ao carregar status: ${err.name || ""} ${err.message || err}${dica}`, "error");
    }
}
...
async function participarAirdrop() {
    ...
    try {
        ...
        if (data.error) {
            const mensagemBase = MENSAGENS_ERRO[data.error] || `Erro: ${data.error}`;
            const mensagemComDetalhe = data.detail ? `${mensagemBase}<br><small>${data.detail}</small>` : mensagemBase;
            setStatus(mensagemComDetalhe, "error");
            airdropBtn.disabled = false;
            return;
        }
